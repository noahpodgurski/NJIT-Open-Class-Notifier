...
global mySections, seatsOpen, COURSELIST, count
...
def chooseClasses():
	global mySections, COURSELIST, sub_btn
	mySections = {}
	root=tk.Tk()
	root.title("NJIT Open Course Notifier")
	...
	def back(event=""):
		global section_labels, section_vars, RUN
		if RUN:
			stop()
	...
	def submit(event=""):
		global sub_btn, section_labels, section_vars, sections, course_entry, course_label, course_var, course_choice, run_btn
		run_btn = tk.Button(root, text = "RUN", command = run)
		run_btn.grid(row=20,column=2)
		section_labels = []
		section_vars = []
		course_choice=course_entry.get()
		# populate sections
		for course in COURSELIST:
...
def main():
	global EMAIL, FROM, PASSWORD, minutes
	# count = 0
	minutes = 1
